...
6. **A second, independent capacity constraint** (shown in-game as "Volume") unlocks past difficulty 75 — empirically, single-constraint instances are already meaningfully hard by around difficulty 50, so the second axis of hardness starts soon after rather than being held back for elite-tier play only. Tightness and the trap are still one algorithmic axis — how informative a value/weight ratio is; a second constraint is a different axis entirely, since no single ratio can rank items once two independent capacities compete. This tracks the multidimensional-knapsack operations-research literature (a documented jump in difficulty passing from 1 to 2+ constraints, requiring different solution machinery than 1D DP) and the multi-attribute decision-making literature (a distinct vmPFC/OFC/ACC/DLPFC integration process versus single-value comparison). Measured: a strategy that only ranks by the first dimension's ratio — the one every other lever trains — drops from ~50% success at unlock to 2-4% within a few hundred difficulty points and holds there.
   ...
   Once unlocked, dim2 applies to exactly 1 in 10 rounds (10%) rather than every round — otherwise it would dominate and crowd out the correlation-regime switching that's the rest of the engine's point. Like the correlation bag, this is a shuffled bag (a 9-false/1-true pool reshuffled each cycle), not a flat 10% coin flip per round, so there's a bounded worst-case gap (~19 rounds) between dim2 rounds instead of leaving exposure to chance. On the 90% of rounds that aren't dim2, the trap still applies once it's separately unlocked past difficulty 350 — the two never combine on the same instance (see "what didn't work"). Value depends on both weight dimensions jointly rather than dimension 2 being independent noise: an early version left dimension-2-only and combined/bottleneck ratio heuristics climbing to 85-98% success (dimension-1-ratio alone stayed hard, but that's not the only heuristic a player might reach for), so weight2 also has to feed value for no single ratio to reliably rank items.
