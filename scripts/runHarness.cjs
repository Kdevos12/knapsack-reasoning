#!/usr/bin/env node
/**
 * Measurement harness runner (CommonJS version).
 * Loads compiled modules from dist/ and runs the audit.
 * Usage: node scripts/runHarness.cjs
 */
...
(async () => {
  try {
    const { difficultyToParams, generateInstance } = await import(
      'file://' + path.join(__dirname, '..', 'dist', 'instanceGenerator.js')
    );
    const { solveKnapsack } = await import(
      'file://' + path.join(__dirname, '..', 'dist', 'knapsackSolver.js')
    );

    /**
     * Greedy-by-ratio heuristic
     */
    function greedyByRatio(instance) {
    ...
    function measureAtDifficulty(difficulty, sampleCount = 20) {
      const baseParams = difficultyToParams(difficulty);
      const params = { ...baseParams, correlation: 'strongly_correlated' };
    ...
    function runAudit() {
      ...
      console.log('Difficulty | nItems | Mechanism | Greedy Exact % | Avg Quality %');
      console.log('-----------|--------|-----------|----------------|---------------');
      ...
      const checkBand = (results, mechanism, bandMin = 30, bandMax = 50) => {
      ...
      console.log('    - TRAP_START_DIFFICULTY, TRAP_GROWTH_RATE for trap');
      console.log('    - DIM2_START_DIFFICULTY, DIM2_GROWTH_RATE for dim2');
      console.log('    - CONFLICT_START_DIFFICULTY, CONFLICT_GROWTH_RATE for conflict\n');
    }
